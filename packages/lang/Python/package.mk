...
post_makeinstall_host() {
  # strip
  chmod 755 $ROOT/$TOOLCHAIN/lib/libpython*.s*
}
...
makeinstall_target() {
  make  -j1 CC="$CC" DESTDIR="$SYSROOT_PREFIX/python2.7" \
        PYTHON_DISABLE_MODULES="$PY_DISABLED_MODULES" \
        PYTHON_MODULES_INCLUDE="$TARGET_INCDIR" \
        PYTHON_MODULES_LIB="$TARGET_LIBDIR" \
        install
  ...
  make  -j1 CC="$CC" DESTDIR="$INSTALL" \
    ...
    cp -P $INSTALL/usr/bin/python*-config $SYSROOT_PREFIX/usr/bin
}

post_makeinstall_target() {
  # strip
  chmod 755 $INSTALL/usr/lib/libpython*.s*
...
#Workaround for cross-compilation failed
  mkdir -p $SYSROOT_PREFIX/usr/lib
    cp -P $INSTALL/usr/lib/libpython*.s* $SYSROOT_PREFIX/usr/lib/.
    cp -PR $INSTALL/usr/lib/python* $SYSROOT_PREFIX/usr/lib
    cp -PR $SYSROOT_PREFIX/python*/usr/lib/pkgconfig $SYSROOT_PREFIX/usr/lib
  mkdir -p $SYSROOT_PREFIX/usr/bin
    cp -P $INSTALL/usr/bin/python* $SYSROOT_PREFIX/usr/bin/.
  mkdir -p $SYSROOT_PREFIX/usr/include
    cp -PR $SYSROOT_PREFIX/python*/usr/include/python* $SYSROOT_PREFIX/usr/include
  mkdir -p $SYSROOT_PREFIX/usr/share
    cp -PR $SYSROOT_PREFIX/python*/usr/share/man $SYSROOT_PREFIX/usr/share

 rm -rf $SYSROOT_PREFIX/python*
}
